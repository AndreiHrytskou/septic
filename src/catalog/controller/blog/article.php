	...
	if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_tag'])) {
		...
		if ($article_info) {
            ...
            if ($article_info['image']) {
                ...
                $data['image'] = $this->model_tool_image->cropsize($article_info['image'], '931', '582');
            } else {
                $data['image'] = null;
            }

            $data['date_added'] = date('d/m/Y', strtotime($article_info['date_added']));
            ...
            $top_products = $this->model_blog_article->getLikesProduct(4);

            $product_data = [];

            $this->load->model('catalog/product');

            foreach ($top_products->rows as $result) {
            ...
            if ($results) {
                foreach ($results as $result) {
                    if ($result['image']) {
                        $image = $this->model_tool_image->cropsize($result['image'], 85, 85);
                    } else {
                        $image = $this->model_tool_image->cropsize('placeholder.png', 85, 85);
                    }
