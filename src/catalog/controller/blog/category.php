...
//		);
        ...
        if ($category_info) {
            ...
            foreach ($results as $result) {
                if ($result['image']) {
                    $image = $this->model_tool_image->cropsize($result['image'], '500', '329');
                } else {
            ...
            if($article_total > $page * $limit){
                $data['next_page'] = $this->url->link('blog/category', 'blog_category_id=' . $this->request->get['blog_category_id'] . $url . '&page=');
            }
            ...
            return $this->load->view('blog/category', $data);
