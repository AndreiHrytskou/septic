...
class ControllerExtensionModuleBlogPopular extends Controller {
	public function index($setting) {
        $data['more_news'] = $this->url->link('blog/category', 'blog_category_id='. 69);
		...
		if ($results) {
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->cropsize($result['image'], 500, 250);
				} else {
					$image = $this->model_tool_image->cropsize('placeholder.png', 500, 250);
				}
