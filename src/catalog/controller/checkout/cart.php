		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			if ($files) {
				foreach ($files as $file) {
                    if($file == '/var/www/html/catalog//controller/extension/total/coupon.php'){
...
//			$this->response->setOutput($this->load->view('error/not_found', $data));
    ...
    public function edit() {
        ...
        if (isset($this->request->post['key']) && isset($this->request->post['quantity'])) {
            ...
            if($json['count'] <= 0){
                $json['total'] = $this->language->get('text_empty');
            }else{
                $this->log->write($this->language->get('text_items'));
                $this->log->write($this->cart->countProducts());
