...
class ControllerProductCategory extends Controller {
	public function category() {
		...
		if ($category_info) {
            ...
            $this->document->addScript('/catalog/view/theme/prostoseptik/assets/js/catalog_show.js');
...
//			);
            ...
            $data['sorts'][] = array(
                'text'  => $this->language->get('text_date_addet'),
                'value' => 'p.date_addet',
                'href'  => $this->url->link('product/category', 'path=' . $this->request->get['path'] . '&sort=date_addet&order=ASC' . $url)
            );
...
//			);
            ...
            $data['paginate']['href'] = null;

            if($product_total > $page * $limit){
                $next_page = (int)$page + 1;

                $data['paginate']['href'] = $this->url->link('product/category', 'path=' . $this->request->get['path'] . $url . '&page=');
            }
			...
			if ($limit && ceil($product_total / $limit) > $page) {
                ...
                $data['page_next'] = $this->url->link('product/category', 'path=' . $category_info['category_id'] . '&page='. ($page + 1), true);
			}

            $data['paginate']['page'] = $pagination->page;
            $data['paginate']['limit'] = $pagination->limit;
            $data['paginate']['total'] = $pagination->total;
...
//			$this->response->setOutput($this->load->view('error/not_found', $data));
    ...
    public function index(){
        $data = [];
        $this->response->setOutput($this->category());
    }
