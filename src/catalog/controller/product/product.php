...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
            ...
            $this->document->addScript('/catalog/view/theme/prostoseptik/assets/js/single-product.js');
            $this->document->addScript('/catalog/view/theme/prostoseptik/assets/js/phoneinput.js');
            $this->document->addScript('/catalog/view/theme/prostoseptik/assets/js/ajax.js');
			...
			$data['tab_advantages'] = $this->language->get('tab_advantages');
...
//---
            $data['advantages'] = html_entity_decode($product_info['advantages'], ENT_QUOTES, 'UTF-8');
...
//
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->cropsize($result['image'], 450,  400)
				);
			}
            ...
            if(count($results) == 0){
                $data['images'][] = array(
                    ...
                    'thumb' => $this->model_tool_image->cropsize($product_info['image'], 450,  400)
                );
            }
            ...
            $attribute_group_left = [];
            $attribute_group_right = [];
            $attribute_group_full = [];

            $data['config_telephone'] = $this->config->get('config_telephone');
            $data['telephone_href'] = preg_replace("/[^+0-9]/", '', $data['config_telephone']);
            ...
            for ($i = 0; $i < count($attribute_group_full); $i++){

                if (is_file(DIR_IMAGE . $attribute_group_full[$i]['icon'])) {
                    $thumb = $attribute_group_full[$i]['icon'];
                } else {
                    $thumb = 'no_image.png';
                }

                $attribute_group_full[$i]['icon'] = array(
                    'link'       => $attribute_group_full[$i]['icon'],
                    'thumb'      => $this->model_tool_image->resize($thumb, 16, 16),
                );

                if($i % 2){
                    $attribute_group_right[] = $attribute_group_full[$i];
                }else{
                    $attribute_group_left[] = $attribute_group_full[$i];
                }
            }

            $data['attribute_groups'] = [];
            array_push($data['attribute_groups'], $attribute_group_left);
            array_push($data['attribute_groups'], $attribute_group_right);
