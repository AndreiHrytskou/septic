...
<?php
	...
	if( ! empty( $settings['refresh_results'] ) && $settings['refresh_results'] == 'using_button' && ! empty( $settings['place_button'] ) ) {
		...
		if( in_array( 'top', $place_button ) ) {
			$buttons['top'][] = sprintf( $button_temp, 'show btn btn-primary btn-xs', $text_button_apply );
		}
		
		if( in_array( 'bottom', $place_button ) ) {
			$buttons['bottom'][] = sprintf( $button_temp, 'show btn btn-primary btn-xs', $text_button_apply );
		}
	}
		...
		<ul class="sidebar__form">
			<?php foreach( $filters as $kfilter => $filter ) { ?>
				...
				<li
					...
					class="amountperson mfilter-filter-item mfilter-<?php echo $filter['type']; ?> mfilter-<?php echo $base_type; ?><?php echo $base_type == 'attribute' ? ' mfilter-attributes' : ( $base_type == 'option' ? ' mfilter-options' : ( $base_type == 'filter' ? ' mfilter-filters' : '' ) ); ?>"
					>
					
					<?php if( $filter['collapsed'] != 'hide_header' && ! empty( $filter['name'] ) && empty( $filter['hide_header'] ) ) { ?>
						<div class="amountperson-title mfilter-heading<?php
					...
					<div class="mfilter-content-opts <?php echo $filter['type'] == "checkbox" ? 'clear-list' : '' ?>"<?php
								...
								<div class="mfilter-options">
											...
											<?php } else if( $filter['type'] == 'cat_checkbox' ) { ?>
												<div class="mfilter-tb">
													...
													<?php foreach( $filter['categories'] as $category ) { ?>
														...
														?>
														<div class="manufacturer-select-item mfilter-option mfilter-tb-as-tr<?php echo $category['cnt'] ? '' : ' mfilter-hide'; ?><?php echo $tmpCatShown == 1 && $tmpCatIdx > $tmpCatShown ? ' mfilter-first-child' : ''; ?>">
									...
									<?php } else if( $filter['type'] == 'price' ) { ?>
										<div class="mfilter-option mfilter-price">
											<div class="amountperson-block-count mfilter-price-inputs">
												<?php echo $getSymbolLeft; ?>
												<input
													id="mfilter-opts-price-min"
													type="text"
													class="amountperson-min form-control"
												...
												<input
													...
													type="text"
													class="amountperson-max form-control"
									...
									<?php } else if( $filter['type'] == 'slider' ) { ?>
										<div class="mfilter-option mfilter-slider">
											<div class="amountperson-block-count mfilter-slider-inputs">
												<input
													class="amountperson-min mfilter-opts-slider-min form-control"
													type="text"
													value=""
													readonly="readonly"
													/>
												<span class="line"></span>
												<input
													class="amountperson-max mfilter-opts-slider-max form-control"
													type="text"
													value=""
													readonly="readonly"
													/>
											...
											<div class="mfilter-slider-container">
												<div class="amountperson-ranges mfilter-slider-slider"></div>
											</div>
									...
									<?php } else if( $filter['type'] == 'rating' ) { ?>
										<div class="mfilter-tb">
											<?php for( $i = 5; $i >= 1; $i-- ) { ?>
												<div class="clear-list-item mfilter-option mfilter-tb-as-tr">
													<div class="mfilter-tb-as-td mfilter-col-input">
														<input
										...
										<div class="mfilter-options-container">
											...
											<?php foreach( $filter['options'] as $option_id => $option ) { if( $option['name'] === '' || isset( $options_tmp[$option['key']] ) ) continue; $options_tmp[$option['key']] = true; ?>
												...
												<div class="clear-list-item mfilter-option mfilter-tb-as-tr">
													...
													<div class="mfilter-tb-as-td text-label">
