...
class anyDSVXLSgratwest { 
    ...
    private $specification = array(
        'name' => "gratwest",
        'id_specification' => "gratwest___1_0_0_0",
        'version' => "1_0_0_0",
        'encoding' => 'UTF-8',
    ...
    private $feed_cache = array(
            'status'=>0,
            'count_nodes'=>1000,
            'offers_file_name_prefix'=>'oanydsvxlsym_gratwest_cache_',
            'categories_file_name_prefix'=>'oanydsvxlsym_gratwest_cats_cache_',
            'offers_file_name_prefix_csv'=>'gratwest_csv_'
    );
    ...
    public function viewData($template_data_on_step_one) {
        ...
        if(!isset($template_data_on_step_one['gratwest_count_product_on_sec'])){
            
            $template_data_on_step_one['gratwest_count_product_on_sec'] = $this->settings['count_product_on_sec'];
            
        }
        
        if(!isset($template_data_on_step_one['gratwest_max_limit'])){
            
            $template_data_on_step_one['gratwest_max_limit'] = $this->settings['max_limit'];
	...
	if(!strstr($xml_check, '<yml_catalog')){
        ...
        $xml_replace = str_replace(
                array('<shop>','</shop>','<offers>','</offers>','<offer ','</offer>','DETAIL_DOP_INFO'), 
                array('<shop2>','</shop2>','<offers2>','</offers2>','<element ','</element>','&amp;lt;hr&amp;gt;'),
                $xml_replace);
        ...
        $this->feed_setting['delete_by_vendor'] = $this->getArrayByDelimeterAndString($template_data['gratwest_delete_by_vendor']);
        
        $option_columns = $this->getArrayByDelimeterAndString($template_data['gratwest_option_columns']);
        
        $attribute_columns = $this->getArrayByDelimeterAndString($template_data['gratwest_attribute_columns']);
        
        $delete_columns = $this->getArrayByDelimeterAndString($template_data['gratwest_delete_columns']);
        ...
        $this->feed_setting['option_value_by_column'] = $template_data['gratwest_option_value_by_column'];
        
        $this->feed_setting['option_type'] = $template_data['gratwest_option_type'];
        
        $this->feed_setting['params_as_column'] = $template_data['gratwest_params_as_column'];
        
        if($template_data['gratwest_max_curl_timeout'] && $template_data['gratwest_max_curl_timeout']!=='' && $template_data['gratwest_max_curl_timeout']<600){
            
            $this->settings['max_curl_timeout'] = $template_data['gratwest_max_curl_timeout']*1000;
            
        }
        
        $this->feed_setting['option_whis_image'] = array();
        
        if(isset($template_data['gratwest_option_whis_image'])){
            
            $this->feed_setting['option_whis_image'] = $this->getArrayByDelimeterAndString($template_data['gratwest_option_whis_image']);
            
        }
        
        $this->feed_setting['gratwest_group_id'] = '';
        
        if(isset($template_data['gratwest_group_id']) && $template_data['gratwest_group_id']!==''){
            
            $this->feed_setting['gratwest_group_id'] = $template_data['gratwest_group_id'];
            
        }
        
        $this->feed_setting['gratwest_group_delete'] = array();
        
        if(isset($template_data['gratwest_group_delete']) && $template_data['gratwest_group_delete']!==''){
            
            $this->feed_setting['gratwest_group_delete']['find'] = $this->getArrayByDelimeterAndString($template_data['gratwest_group_delete']);
            
            $this->feed_setting['gratwest_group_delete']['replace'] = array();
            
        }
        
        $this->feed_setting['gratwest_group_id_write_to_id'] = FALSE;
        
        if(isset($template_data['gratwest_group_id_write_to_id']) && $template_data['gratwest_group_id_write_to_id']){
            
            $this->feed_setting['gratwest_group_id_write_to_id'] = TRUE;
            
        }
        
        $this->feed_setting['gratwest_product_id_column_name'] = '';
        
        if(isset($template_data['gratwest_product_id_column_name']) && $template_data['gratwest_product_id_column_name']!==''){
            
            $this->feed_setting['gratwest_product_id_column_name'] = $template_data['gratwest_product_id_column_name'];
            
        }
        
        $this->feed_setting['gratwest_unit_not_write'] = FALSE;
        
        if(isset($template_data['gratwest_unit_not_write']) && $template_data['gratwest_unit_not_write']){
            
            $this->feed_setting['gratwest_unit_not_write'] = TRUE;
            
        }
        
        
        $this->feed_setting['select_params_as_column'] = $this->getArrayByDelimeterAndString($template_data['gratwest_select_params_as_column']);
        
        $this->feed_setting['delete_params'] = $this->getArrayByDelimeterAndString($template_data['gratwest_delete_params']);
        
        $this->feed_setting['start_offer'] = (int)$template_data['gratwest_start_offer'];
        ...
        $this->feed_setting['limit_offer'] = trim($template_data['gratwest_limit_offer']);
    ...
    public function getCSVFromSource($filename,$template_data,$start=0,$limit=30){
        ...
        $gratwest_categories_as_string = array();
        
        if(isset($template_data['gratwest_categories_as_string']) && $template_data['gratwest_categories_as_string']!==''){
            
            $gratwest_categories_as_string = explode(PHP_EOL, $template_data['gratwest_categories_as_string']);
            
            foreach ($gratwest_categories_as_string as $key => $value) {
                
                $gratwest_categories_as_string[$key] = html_entity_decode(trim(strtoupper($value)));
                
                if(!$gratwest_categories_as_string[$key]){
                    unset($gratwest_categories_as_string[$key]);
                }
                ...
                if(!$error && isset($product['code'])){
                    ...
                    foreach ($product as $field => $value) {
                        ...
                        if($field=='category'){
                            ...
                            if($gratwest_categories_as_string){
                                
                                $skip_by_category = TRUE;
                                
                                foreach ($gratwest_categories_as_string as $category) {
    ...
    public function getCMLParams($filename,$template_data,$tags,$count_nodes=1,$tags_new_names=array(),$start=0,$limit=100000,$file_name_cach_prefix,$selected_setting_id=''){
        ...
        /*
        $this->feed_setting['delete_by_category_id'] = $this->getArrayByDelimeterAndString($template_data['gratwest_delete_by_category_id']);
    ...
    public function getYMLCategories($filename,$template_data,$tags,$count_nodes=1,$tags_new_names=array(),$start=0,$limit=100000,$file_name_cach_prefix,$selected_setting_id=''){
        ...
        $this->feed_setting['set_products_by_category_id'] = $this->getArrayByDelimeterAndString($template_data['gratwest_set_products_by_category_id']);
    ...
    public function getValueFromCDATA($cdata,$strip_tags=TRUE) {
        
        if(!$strip_tags){
            return htmlspecialchars(str_replace(array('<![CDATA[ ',' ]]>'),array('',''),   html_entity_decode(trim((string)$cdata))),ENT_QUOTES);
        }else{
    ...
    private function getDsvRowFromOfferNode($offer_row,$only_first_row=FALSE,$offers_file_name_prefix){
            ...
            $write_param_as_columns = array('Количество','Наличие',"Артикул","Штрих-код","Торговая марка","Цена","Код","Максимальная скидка","Минимальная скидка","Акция на сайте","Описание товара","Доп. информация номенклатуры");
            ...
            if(isset($offer_row->param) && !$supplier_country_id){
                ...
                foreach ($offer_row->param as $yml_param) {
                    ...
                    if(isset($attributes['@attributes']['name']) && in_array($attributes['@attributes']['name'], $write_param_as_columns)){
                        ...
                        $offer_additional_colums[$attributes['@attributes']['name']] = trim($this->getValueFromCDATA($yml_param,FALSE));
                ...
                foreach ($offer_row->param as $yml_param) {
                    ...
                    foreach ($attributes['@attributes'] as $attribute_name => $attribute_value) {
                        ...
                        if($attribute_name=='name' && !in_array((string)$attribute_value, $special_params)){
                            ...
                            if(isset($attributes['@attributes']['unit']) && !$this->feed_setting['gratwest_unit_not_write']){
            ...
            if(isset($offer_row->name)){
                ...
                $name = $this->getValueFromCDATA($offer_row->name,FALSE);
            ...
            if(isset($offer_row->description)){
                ...
                if(strstr((string)$offer_row->description, '[CDATA[')){
                    ...
                    $description = $this->getValueFromCDATA($offer_row->description,FALSE);
            ...
            $column_as_params_whis_group_this = array(
                ...
                'width'=>array('group'=>'Характеристики шины','attribute'=>'Ширина профиля, мм'),
                'h'=>array('group'=>'Характеристики шины','attribute'=>'Высота профиля, мм'),
                'radius'=>array('group'=>'Характеристики шины','attribute'=>'Диаметр'),
                'season'=>array('group'=>'Характеристики шины','attribute'=>'Сезонность'),
                'stud'=>array('group'=>'Характеристики шины','attribute'=>'Шипы'),
                'ins'=>array('group'=>'Характеристики шины','attribute'=>'Индекс'),
            );
            ...
            if($this->feed_setting['gratwest_group_id'] && isset($this->feed_setting['offers'][$id][$this->feed_setting['gratwest_group_id']]) && $this->feed_setting['offers'][$id][$this->feed_setting['gratwest_group_id']]!==''){
                
                $this_group_offer = $this->feed_setting['offers'][$id];
                
                $new_id_from_group_id = $this->feed_setting['offers'][$id][$this->feed_setting['gratwest_group_id']];
                
                if($this->feed_setting['gratwest_group_delete'] && isset($this_group_offer['name'])){
                    
                    $this_group_offer['name'] = str_replace($this->feed_setting['gratwest_group_delete']['find'], $this->feed_setting['gratwest_group_delete']['replace'], $this_group_offer['name']);
                ...
                if($this->feed_setting['gratwest_group_id_write_to_id'] && $this->feed_setting['gratwest_product_id_column_name']!==''){
                    
                    $this_group_offer[$this->feed_setting['gratwest_product_id_column_name']] = $new_id_from_group_id;
