...
class anyDSVXLSetim {
    ...
    private $specification = array(
        'name' => "etim",
        'id_specification' => "etim___1_0_0_0",
        'version' => "1_0_0_0",
        'specification_valid_text'=>'<Каталог ',
        'encoding' => 'UTF-8',
    );
    private $feed_cache = array(
            'status'=>0,
            'count_nodes'=>1000,
            'offers_file_name_prefix'=>'oanydsvxlsym_etim_cache_',
            'categories_file_name_prefix'=>'oanydsvxlsym_etim_cats_cache_',
            'offers_file_name_prefix_csv'=>'etim_csv_'
    );
    ...
    public function viewData($template_data_on_step_one) {
        ...
        if(!isset($template_data_on_step_one['etim_count_product_on_sec'])){
            
            $template_data_on_step_one['etim_count_product_on_sec'] = $this->settings['count_product_on_sec'];
            
        }
        
        if(!isset($template_data_on_step_one['etim_max_limit'])){
            
            $template_data_on_step_one['etim_max_limit'] = $this->settings['max_limit'];
	...
	if(!strstr($xml_check, '<Каталог ')){
	    
	    $result['error'] = "Данный файл не является файлом XML или спецификацией ETIM. Проверьте правильность ссылки или файла";
        ...
        $xml_replace = str_replace(array('<Каталог','</Каталог>','<Товары>','</Товары>','<Товар>','</Товар>'), array('<yml_catalog><shop2','</shop2></yml_catalog>','<offers2>','</offers2>','<element>','</element>'), $xml_replace);
        ...
        $this->feed_setting['delete_by_vendor'] = $this->getArrayByDelimeterAndString($template_data['etim_delete_by_vendor']);
        
        $option_columns = $this->getArrayByDelimeterAndString($template_data['etim_option_columns']);
        
        $attribute_columns = $this->getArrayByDelimeterAndString($template_data['etim_attribute_columns']);
        
        $delete_columns = $this->getArrayByDelimeterAndString($template_data['etim_delete_columns']);
        ...
        $this->feed_setting['option_value_by_column'] = $template_data['etim_option_value_by_column'];
        
        $this->feed_setting['option_type'] = $template_data['etim_option_type'];
        
        $this->feed_setting['params_as_column'] = $template_data['etim_params_as_column'];
        
        if($template_data['etim_max_curl_timeout'] && $template_data['etim_max_curl_timeout']!=='' && $template_data['etim_max_curl_timeout']<600){
            
            $this->settings['max_curl_timeout'] = $template_data['etim_max_curl_timeout']*1000;
            
        }
        
        $this->feed_setting['option_whis_image'] = array();
        
        if(isset($template_data['etim_option_whis_image'])){
            
            $this->feed_setting['option_whis_image'] = $this->getArrayByDelimeterAndString($template_data['etim_option_whis_image']);
            
        }
        
        $this->feed_setting['etim_group_id'] = '';
        
        if(isset($template_data['etim_group_id']) && $template_data['etim_group_id']!==''){
            
            $this->feed_setting['etim_group_id'] = $template_data['etim_group_id'];
            
        }
        
        $this->feed_setting['etim_group_delete'] = array();
        
        if(isset($template_data['etim_group_delete']) && $template_data['etim_group_delete']!==''){
            
            $this->feed_setting['etim_group_delete']['find'] = $this->getArrayByDelimeterAndString($template_data['etim_group_delete']);
            
            $this->feed_setting['etim_group_delete']['replace'] = array();
            
        }
        
        $this->feed_setting['etim_group_id_write_to_id'] = FALSE;
        
        if(isset($template_data['etim_group_id_write_to_id']) && $template_data['etim_group_id_write_to_id']){
            
            $this->feed_setting['etim_group_id_write_to_id'] = TRUE;
            
        }
        
        $this->feed_setting['etim_product_id_column_name'] = '';
        
        if(isset($template_data['etim_product_id_column_name']) && $template_data['etim_product_id_column_name']!==''){
            
            $this->feed_setting['etim_product_id_column_name'] = $template_data['etim_product_id_column_name'];
            
        }
        
        $this->feed_setting['etim_unit_not_write'] = FALSE;
        
        if(isset($template_data['etim_unit_not_write']) && $template_data['etim_unit_not_write']){
            
            $this->feed_setting['etim_unit_not_write'] = TRUE;
            
        }
        
        
        $this->feed_setting['select_params_as_column'] = $this->getArrayByDelimeterAndString($template_data['etim_select_params_as_column']);
        
        $this->feed_setting['delete_params'] = $this->getArrayByDelimeterAndString($template_data['etim_delete_params']);
        
        $this->feed_setting['start_offer'] = (int)$template_data['etim_start_offer'];
        ...
        $this->feed_setting['limit_offer'] = trim($template_data['etim_limit_offer']);
    ...
    public function getCSVFromSource($filename,$template_data,$start=0,$limit=30){
        ...
        $etim_categories_as_string = array();
        
        if(isset($template_data['etim_categories_as_string']) && $template_data['etim_categories_as_string']!==''){
            
            $etim_categories_as_string = explode(PHP_EOL, $template_data['etim_categories_as_string']);
            
            foreach ($etim_categories_as_string as $key => $value) {
                
                $etim_categories_as_string[$key] = html_entity_decode(trim(strtoupper($value)));
                
                if(!$etim_categories_as_string[$key]){
                    unset($etim_categories_as_string[$key]);
                }
                ...
                if(!$error && isset($product['code'])){
                    ...
                    foreach ($product as $field => $value) {
                        ...
                        if($field=='category'){
                            ...
                            if($etim_categories_as_string){
                                
                                $skip_by_category = TRUE;
                                
                                foreach ($etim_categories_as_string as $category) {
    ...
    public function getCMLParams($filename,$template_data,$tags,$count_nodes=1,$tags_new_names=array(),$start=0,$limit=100000,$file_name_cach_prefix,$selected_setting_id=''){
        ...
        /*
        $this->feed_setting['delete_by_category_id'] = $this->getArrayByDelimeterAndString($template_data['etim_delete_by_category_id']);
    ...
    private function getDsvRowFromOfferNode($offer_row,$only_first_row=FALSE,$offers_file_name_prefix){
                ...
                foreach ($offer_row->param as $yml_param) {
                    ...
                    foreach ($attributes['@attributes'] as $attribute_name => $attribute_value) {
                        ...
                        if($attribute_name=='name' && !in_array((string)$attribute_value, $special_params)){
                            ...
                            if(isset($attributes['@attributes']['unit']) && !$this->feed_setting['etim_unit_not_write']){
            ...
            $description_new_row = array();
            
            if(isset($offer_row->{'ОписаниеГруппы'})){
                
                foreach ($offer_row->{'ОписаниеГруппы'} as $name_val => $val) {
                
                    $n = 0;
                    
                    foreach ($val as $name_val2 => $val2) {
                        
                        if(!$n){
                            
                            $description_new_row['name'] = $this->getValueFromCDATA($val2);
                            
                            $n++;
                            
                        }else{
                            
                            if(!isset($description_new[$description_new_row['name']])){
                                
                                $description_new[$description_new_row['name']]['val'] = array();
                                
                            }
                            
                            $description_new[$description_new_row['name']]['val'][] = $this->getValueFromCDATA($val2);
            ...
            if($description_new){
                ...
                $description_new_tmp = $description_new;
                
                $description_new = '';
                
                foreach ($description_new_tmp as $title => $lis) {
                    
                    if(count($lis['val'])>1){
                    
                        $description_new .= "<h3>".$title."</h3><ul>";
                        
                        foreach ($lis['val'] as $li) {
                            
                            $description_new .= "<li>".$li."</li>";
                    ...
                    elseif($lis['val']){
                        ...
                        $description_new .= "<h3>".$title."</h3><p>". $lis['val'][0]."</p>";
            ...
            if(isset($offer_row->{'ПараметрыETIM'})){
                
                $param_new_nodes = $offer_row->{'ПараметрыETIM'};
                
                foreach ($param_new_nodes as $param_new_node) {
                    
                    if(isset($param_new_node->{'ПараметрETIM'})){
                        
                        foreach ($param_new_node->{'ПараметрETIM'} as $param_new_node2) {
                            
                            $column_as_params_o_name = '';
                    
                            $column_as_params_t_value = '';
                            
                            if(isset($param_new_node2->{'Наименование'})){
                            
                                $column_as_params_o_name = $this->getValueFromCDATA($param_new_node2->{'Наименование'});

                                if(isset($param_new_node2->{'ЕдиницаИзмерения'})){

                                    $column_as_params_o_name .= $this->getValueFromCDATA($param_new_node2->{'ЕдиницаИзмерения'});

                                }

                                if($param_new_node2->{'Значение'}){

                                    $column_as_params_t_value = $this->getValueFromCDATA($param_new_node2->{'Значение'});

                                }

                            }
                            
                            if($column_as_params_o_name){

                                $params_this = array('title'=>$column_as_params_o_name,'group_title'=>'Параметры','value'=>$column_as_params_t_value,'image'=>'','feature_code'=>'','product_feature_code'=>'');

                                $params_new[] = $params_this;
            ...
            $params = array_merge($params_new,$params);
            
            $params_new = array();
            
            if(isset($offer_row->{'ТехническиеХарактеристики'})){
                
                $param_new_nodes = $offer_row->{'ТехническиеХарактеристики'};
                
                foreach ($param_new_nodes as $param_new_node) {
                    
                    if(isset($param_new_node->{'ТехническаяХарактеристика'})){
                        
                        foreach ($param_new_node->{'ТехническаяХарактеристика'} as $param_new_node2) {
                            ...
                            if($column_as_params_o_name){
                                ...
                                $params_this = array('title'=>$column_as_params_o_name,'group_title'=>'Технические характеристики','value'=>$column_as_params_t_value,'image'=>'','feature_code'=>'','product_feature_code'=>'');
            ...
            if(isset($offer_row->{'ВесогабаритныеХарактеристики'})){
                
                $param_new_nodes = $offer_row->{'ВесогабаритныеХарактеристики'};
                
                foreach ($param_new_nodes as $param_new_node) {
                    
                    if(isset($param_new_node->{'ВесогабаритнаяХарактеристика'})){
                        
                        foreach ($param_new_node->{'ВесогабаритнаяХарактеристика'} as $param_new_node2) {
                            ...
                            if($column_as_params_o_name){
                                ...
                                $params_this = array('title'=>$column_as_params_o_name,'group_title'=>'Габариты и вес','value'=>$column_as_params_t_value,'image'=>'','feature_code'=>'','product_feature_code'=>'');
            ...
            if(isset($offer_row->{'Группы'})){
                
                $groups_nodes = $offer_row->{'Группы'};
                
                foreach ($groups_nodes->{'Ид'} as $param_new_node) {
                    
                    $groups[] = $this->getValueFromCDATA($param_new_node );
            ...
            $this->feed_setting['offers'][$id]['ГруппИд'] = implode(',', $groups);
            ...
            $this->feed_setting['offers'][$id]['description_new'] = $description_new;
            ...
            if($this->feed_setting['etim_group_id'] && isset($this->feed_setting['offers'][$id][$this->feed_setting['etim_group_id']]) && $this->feed_setting['offers'][$id][$this->feed_setting['etim_group_id']]!==''){
                
                $this_group_offer = $this->feed_setting['offers'][$id];
                
                $new_id_from_group_id = $this->feed_setting['offers'][$id][$this->feed_setting['etim_group_id']];
                
                if($this->feed_setting['etim_group_delete'] && isset($this_group_offer['name'])){
                    
                    $this_group_offer['name'] = str_replace($this->feed_setting['etim_group_delete']['find'], $this->feed_setting['etim_group_delete']['replace'], $this_group_offer['name']);
                ...
                if($this->feed_setting['etim_group_id_write_to_id'] && $this->feed_setting['etim_product_id_column_name']!==''){
                    
                    $this_group_offer[$this->feed_setting['etim_product_id_column_name']] = $new_id_from_group_id;
