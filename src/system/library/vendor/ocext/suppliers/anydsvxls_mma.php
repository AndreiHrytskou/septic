...
class anyDSVXLSmma {
    ...
    private $specification = array(
        'name' => "mma",
        'id_specification' => "mma___1_0_0_0",
        'version' => "1_0_0_0",
        'encoding' => 'UTF-8',
    ...
    private $feed_cache = array(
            'status'=>0,
            'count_nodes'=>1000,
            'offers_file_name_prefix'=>'oanydsvxlsym_mma_cache_',
            'categories_file_name_prefix'=>'oanydsvxlsym_mma_cats_cache_',
            'offers_file_name_prefix_csv'=>'mma_csv_'
    );
    ...
    public function viewData($template_data_on_step_one) {
        ...
        if(!isset($template_data_on_step_one['mma_count_product_on_sec'])){
            
            $template_data_on_step_one['mma_count_product_on_sec'] = $this->settings['count_product_on_sec'];
            
        }
        
        if(!isset($template_data_on_step_one['mma_max_limit'])){
            
            $template_data_on_step_one['mma_max_limit'] = $this->settings['max_limit'];
	...
	if(!strstr($xml_check, '<price>')){
        ...
        $xml_replace = str_replace(
                array('<yml_catalog>','</yml_catalog>','<price>','</price>','<item ','</item>','<items>','</items>','<catalog>','</catalog>'), 
                array('<categories>','</categories>','<yml_catalog><shop2>','</shop2></yml_catalog>','<element ','</element>','<offers2>','</offers2>','<categories>','</categories>'),
                $xml_replace);
        ...
        $this->feed_setting['delete_by_vendor'] = $this->getArrayByDelimeterAndString($template_data['mma_delete_by_vendor']);
        
        $option_columns = $this->getArrayByDelimeterAndString($template_data['mma_option_columns']);
        
        $attribute_columns = $this->getArrayByDelimeterAndString($template_data['mma_attribute_columns']);
        
        $delete_columns = $this->getArrayByDelimeterAndString($template_data['mma_delete_columns']);
        ...
        $this->feed_setting['option_value_by_column'] = $template_data['mma_option_value_by_column'];
        
        $this->feed_setting['option_type'] = $template_data['mma_option_type'];
        
        $this->feed_setting['params_as_column'] = $template_data['mma_params_as_column'];
        
        if($template_data['mma_max_curl_timeout'] && $template_data['mma_max_curl_timeout']!=='' && $template_data['mma_max_curl_timeout']<600){
            
            $this->settings['max_curl_timeout'] = $template_data['mma_max_curl_timeout']*1000;
            
        }
        
        $this->feed_setting['option_whis_image'] = array();
        
        if(isset($template_data['mma_option_whis_image'])){
            
            $this->feed_setting['option_whis_image'] = $this->getArrayByDelimeterAndString($template_data['mma_option_whis_image']);
            
        }
        
        $this->feed_setting['mma_group_id'] = '';
        
        if(isset($template_data['mma_group_id']) && $template_data['mma_group_id']!==''){
            
            $this->feed_setting['mma_group_id'] = $template_data['mma_group_id'];
            
        }
        
        $this->feed_setting['mma_group_delete'] = array();
        
        if(isset($template_data['mma_group_delete']) && $template_data['mma_group_delete']!==''){
            
            $this->feed_setting['mma_group_delete']['find'] = $this->getArrayByDelimeterAndString($template_data['mma_group_delete']);
            
            $this->feed_setting['mma_group_delete']['replace'] = array();
            
        }
        
        $this->feed_setting['mma_group_id_write_to_id'] = FALSE;
        
        if(isset($template_data['mma_group_id_write_to_id']) && $template_data['mma_group_id_write_to_id']){
            
            $this->feed_setting['mma_group_id_write_to_id'] = TRUE;
            
        }
        
        $this->feed_setting['mma_product_id_column_name'] = '';
        
        if(isset($template_data['mma_product_id_column_name']) && $template_data['mma_product_id_column_name']!==''){
            
            $this->feed_setting['mma_product_id_column_name'] = $template_data['mma_product_id_column_name'];
            
        }
        
        $this->feed_setting['mma_unit_not_write'] = FALSE;
        
        if(isset($template_data['mma_unit_not_write']) && $template_data['mma_unit_not_write']){
            
            $this->feed_setting['mma_unit_not_write'] = TRUE;
            
        }
        
        
        $this->feed_setting['select_params_as_column'] = $this->getArrayByDelimeterAndString($template_data['mma_select_params_as_column']);
        
        $this->feed_setting['delete_params'] = $this->getArrayByDelimeterAndString($template_data['mma_delete_params']);
        
        $this->feed_setting['start_offer'] = (int)$template_data['mma_start_offer'];
        ...
        $this->feed_setting['limit_offer'] = trim($template_data['mma_limit_offer']);
    ...
    public function getCSVFromSource($filename,$template_data,$start=0,$limit=30){
        ...
        $mma_categories_as_string = array();
        
        if(isset($template_data['mma_categories_as_string']) && $template_data['mma_categories_as_string']!==''){
            
            $mma_categories_as_string = explode(PHP_EOL, $template_data['mma_categories_as_string']);
            
            foreach ($mma_categories_as_string as $key => $value) {
                
                $mma_categories_as_string[$key] = html_entity_decode(trim(strtoupper($value)));
                
                if(!$mma_categories_as_string[$key]){
                    unset($mma_categories_as_string[$key]);
                }
                ...
                if(!$error && isset($product['code'])){
                    ...
                    foreach ($product as $field => $value) {
                        ...
                        if($field=='category'){
                            ...
                            if($mma_categories_as_string){
                                
                                $skip_by_category = TRUE;
                                
                                foreach ($mma_categories_as_string as $category) {
    ...
    public function getCMLParams($filename,$template_data,$tags,$count_nodes=1,$tags_new_names=array(),$start=0,$limit=100000,$file_name_cach_prefix,$selected_setting_id=''){
        ...
        /*
        $this->feed_setting['delete_by_category_id'] = $this->getArrayByDelimeterAndString($template_data['mma_delete_by_category_id']);
    ...
    public function getYMLCategories($filename,$template_data,$tags,$count_nodes=1,$tags_new_names=array(),$start=0,$limit=100000,$file_name_cach_prefix,$selected_setting_id=''){
        ...
        $this->feed_setting['set_products_by_category_id'] = $this->getArrayByDelimeterAndString($template_data['mma_set_products_by_category_id']);
    ...
    private function getDsvRowFromOfferNode($offer_row,$only_first_row=FALSE,$offers_file_name_prefix){
                ...
                foreach ($offer_row->param as $yml_param) {
                    ...
                    foreach ($attributes['@attributes'] as $attribute_name => $attribute_value) {
                        ...
                        if($attribute_name=='name' && !in_array((string)$attribute_value, $special_params)){
                            ...
                            if(isset($attributes['@attributes']['unit']) && !$this->feed_setting['mma_unit_not_write']){
            ...
            if($this->feed_setting['mma_group_id'] && isset($this->feed_setting['offers'][$id][$this->feed_setting['mma_group_id']]) && $this->feed_setting['offers'][$id][$this->feed_setting['mma_group_id']]!==''){
                
                $this_group_offer = $this->feed_setting['offers'][$id];
                
                $new_id_from_group_id = $this->feed_setting['offers'][$id][$this->feed_setting['mma_group_id']];
                
                if($this->feed_setting['mma_group_delete'] && isset($this_group_offer['name'])){
                    
                    $this_group_offer['name'] = str_replace($this->feed_setting['mma_group_delete']['find'], $this->feed_setting['mma_group_delete']['replace'], $this_group_offer['name']);
                ...
                if($this->feed_setting['mma_group_id_write_to_id'] && $this->feed_setting['mma_product_id_column_name']!==''){
                    
                    $this_group_offer[$this->feed_setting['mma_product_id_column_name']] = $new_id_from_group_id;
