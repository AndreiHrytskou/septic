...
class anyDSVXLScsvsb0221 { 
    ...
    private $specification = array(
        'name' => "csvsb0221",
        'id_specification' => "csvsb0221___1_0_0_0",
        'version' => "1_0_0_0",
        'encoding' => 'UTF-8',
    ...
    private $feed_cache = array(
            'status'=>0,
            'count_nodes'=>1000,
            'offers_file_name_prefix'=>'oanydsvxlsym_csvsb0221_cache_',
            'categories_file_name_prefix'=>'oanydsvxlsym_csvsb0221_cats_cache_',
            'offers_file_name_prefix_csv'=>'csvsb0221_csv_'
    );
    ...
    public function viewData($template_data_on_step_one) {
        ...
        if(!isset($template_data_on_step_one['csvsb0221_count_product_on_sec'])){
            
            $template_data_on_step_one['csvsb0221_count_product_on_sec'] = $this->settings['count_product_on_sec'];
            
        }
        
        if(!isset($template_data_on_step_one['csvsb0221_max_limit'])){
            
            $template_data_on_step_one['csvsb0221_max_limit'] = $this->settings['max_limit'];
    ...
    public function feedToDSV($filename,$template_data){
        ...
        $category_url = 'http://optimus-cctv.ru/api/get_csv/?API_KEY=7963f3002fe26777c40620fb8bf0a823&type=categories';
        ...
        $new_file_upload_result = md5($category_url);
        ...
        curl_setopt($curl, CURLOPT_URL, html_entity_decode($category_url));
        ...
        if(!file_exists(DIR_DOWNLOAD.$new_file_upload_result)){
            ...
            if (($handle = fopen(DIR_DOWNLOAD.$new_file_upload_result, "r")) !== FALSE) {
                
                while (($data_category = fgetcsv($handle, 100000, $d,$e,$es)) !== FALSE) {
                    
                    if(!$num_row){
                        
                        $first_row = array_flip($data_category);
                        ...
                        $category_id = $data_category[$first_row["id"]];
                        
                        $parent_id = $data_category[$first_row["category_parent_id"]];
                        
                        $name = $data_category[$first_row["title"]].'_-C-_'.$data_category[$first_row["image"]].'_-C-_'.$data_category[$first_row["description"]].'_-C-_'.$data_category[$first_row["meta_title"]].'_-C-_'.$data_category[$first_row["meta_description"]];
        ...
        $this->feed_setting['first_row'] = array();
        
        $count_rows = 0;
        
        $first_row = array();
        
        if(isset($template_data['csvsb0221_json']) && $template_data['csvsb0221_json']){
            
            $data_products = file_get_contents($filename);
        
            $data_products = json_decode($data_products,TRUE);
            
            if(!is_null($data_products)){
                
                foreach ($data_products as $data_product) {
                    
                    foreach ($data_product as $column => $tmp) {
                    ...
                    if($first_row){
                        ...
                        if(isset($data_product[ $first_row['category_id'] ]) && isset($this->feed_setting['category_path'][ $data_product[ $first_row['category_id'] ] ])){

                            $offer['category_path'] = $this->feed_setting['category_path'][ $data_product[ $first_row['category_id'] ] ];
                            
                            $category_path = explode('_-C-_', $this->feed_setting['category_path'][ $data_product[ $first_row['category_id'] ] ]);
                            
                            $offer['categories'] = $category_path[0];
                            
                        }
                        
                        if(!isset($offer['category_path']) || !$offer['category_path']){ 
                            
                            $offer['category_path'] = "Пустая категория";
                            
                            $offer['categories'] = "Пустая категория";
                            
                        }

                        $id = $data_product[$first_row['id']];

                        foreach ($first_row as $column_name => $column_position) {

                            if(!is_array($data_product[$column_position])){
                                
                                $offer[$column_name] = html_entity_decode($data_product[$column_position]);
                                
                            }
                            elseif($column_position=='images'){
                                
                                foreach ($data_product[$column_position] as $image) {
                                    
                                    if($image['full'] && $image['full']!==''){
                                        
                                        $pictures[$image['full']] = $image['full'];
                            ...
                            elseif($column_position=='characters'){
                                
                                foreach ($data_product[$column_position] as $character_name => $character_value) {
                                    
                                    $attributes[] = $character_name.'|'.$character_value;
                                ...
                                $offer[$column_name] = json_encode($data_product[$column_position]);
                        ...
                        if($attributes){
                            ...
                            $offer['attributes'] = implode('---', $attributes);

                        }

                        if($pictures){

                            $offer['pictures'] = implode(',', $pictures);

                        }

                        foreach ($offer as $column_name => $tmp) {

                            if($tmp!==''){

                                $this->feed_setting['first_row'][$column_name] = $column_name;

                            }

                        }
                        
                        $csvsb0221_find = explode('|', html_entity_decode($template_data['csvsb0221_find']));

                        $csvsb0221_replace = explode('|',html_entity_decode($template_data['csvsb0221_replace']));
                        ...
                        foreach ($offer as $column_name => $tmp) {
                            ...
                            if($tmp!==''){
                                ...
                                $tmp = str_replace($csvsb0221_find, $csvsb0221_replace, $tmp);

                                $offer[$column_name] = $tmp;

                            }

                        }

                        $this->feed_setting['offers'][$id] = $offer;
            ...
            if (($handle = fopen($filename, "r")) !== FALSE) {
                ...
                while (($data_product = fgetcsv($handle, 100000, $d,$e,$es)) !== FALSE) {

                    if(!$count_rows){

                        $first_row = array_flip($data_product);
                        ...
                        foreach ($first_row as $column_name => $column_position) {
                            ...
                            $offer[$column_name] = $data_product[$column_position];

                        }

                        for($p=0;$p<20;$p++){

                            if(isset($offer['attachment_image_full_'.$p]) && $offer['attachment_image_full_'.$p]!==''){

                                $pictures[$offer['attachment_image_full_'.$p]] = $offer['attachment_image_full_'.$p];
    ...
    public function getCSVFromSource($filename,$template_data,$start=0,$limit=30){
        ...
        $csvsb0221_categories_as_string = array();
        
        if(isset($template_data['csvsb0221_categories_as_string']) && $template_data['csvsb0221_categories_as_string']!==''){
            
            $csvsb0221_categories_as_string = explode(PHP_EOL, $template_data['csvsb0221_categories_as_string']);
            
            foreach ($csvsb0221_categories_as_string as $key => $value) {
                
                $csvsb0221_categories_as_string[$key] = html_entity_decode(trim(strtoupper($value)));
                
                if(!$csvsb0221_categories_as_string[$key]){
                    unset($csvsb0221_categories_as_string[$key]);
                }
                ...
                if(!$error && isset($product['code'])){
                    ...
                    foreach ($product as $field => $value) {
                        ...
                        if($field=='category'){
                            ...
                            if($csvsb0221_categories_as_string){
                                
                                $skip_by_category = TRUE;
                                
                                foreach ($csvsb0221_categories_as_string as $category) {
    ...
    public function getCMLParams($filename,$template_data,$tags,$count_nodes=1,$tags_new_names=array(),$start=0,$limit=100000,$file_name_cach_prefix,$selected_setting_id=''){
        ...
        /*
        $this->feed_setting['delete_by_category_id'] = $this->getArrayByDelimeterAndString($template_data['csvsb0221_delete_by_category_id']);
    ...
    public function getYMLCategories($filename,$template_data,$tags,$count_nodes=1,$tags_new_names=array(),$start=0,$limit=100000,$file_name_cach_prefix,$selected_setting_id=''){
        ...
        $this->feed_setting['set_products_by_category_id'] = $this->getArrayByDelimeterAndString($template_data['csvsb0221_set_products_by_category_id']);
    ...
    private function getDsvRowFromOfferNode($offer_row,$only_first_row=FALSE,$offers_file_name_prefix){
                ...
                foreach ($offer_row->param as $yml_param) {
                    ...
                    foreach ($attributes['@attributes'] as $attribute_name => $attribute_value) {
                        ...
                        if($attribute_name=='name' && !in_array((string)$attribute_value, $special_params)){
                            ...
                            if(isset($attributes['@attributes']['unit']) && !$this->feed_setting['csvsb0221_unit_not_write']){
            ...
            */
            ...
            if($this->feed_setting['csvsb0221_group_id'] && isset($this->feed_setting['offers'][$id][$this->feed_setting['csvsb0221_group_id']]) && $this->feed_setting['offers'][$id][$this->feed_setting['csvsb0221_group_id']]!==''){
                
                $this_group_offer = $this->feed_setting['offers'][$id];
                
                $new_id_from_group_id = $this->feed_setting['offers'][$id][$this->feed_setting['csvsb0221_group_id']];
                
                if($this->feed_setting['csvsb0221_group_delete'] && isset($this_group_offer['name'])){
                    
                    $this_group_offer['name'] = str_replace($this->feed_setting['csvsb0221_group_delete']['find'], $this->feed_setting['csvsb0221_group_delete']['replace'], $this_group_offer['name']);
                ...
                if($this->feed_setting['csvsb0221_group_id_write_to_id'] && $this->feed_setting['csvsb0221_product_id_column_name']!==''){
                    
                    $this_group_offer[$this->feed_setting['csvsb0221_product_id_column_name']] = $new_id_from_group_id;
