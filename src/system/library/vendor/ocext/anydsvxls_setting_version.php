<?php
function getOCextNumbersGensetProfIds(){
    return $numbers_genset_prof_id = array(700,1300,150);
}
...
define('anycsvsev_license_cancelled', 'Лицензия отменена и больше неактивна');
define('anycsvsev_empty_key_or_email', 'Невозможно активировать. Не задан ключ или email');
define('anycsvsev_curl_function_exists', 'Невозможно активировать. На хостинге выключено расширение CURL');
define('anycsvsev_license_success_1', 'Лицензия активна');
define('anycsvsev_license_success_2', 'Лицензия активирована');
define('anycsvsev_license_failed_1', 'Один из файлов лицензии поврежден, пожалуйста, переустановите модуль');
define('anycsvsev_license_failed_2', 'Неизвестная ошибка: ');
define('anycsvsev_license_failed_3', 'Неизвестная ошибка, свяжитесь с поддержкой для получения ключа');
define('anycsvsev_license_not_found', 'Невозможно активировать неверный или несуществующий ключ');
define('anycsvsev_license_expired', 'Время действия лицензии истекло, требуется продление');
...
class anyDSVXLSSettingVersion extends setting_part_0 {
    ...
    public function getAbstractFields(){
        ...
        $abstract_field = array(
                ...
                'product_option_value'=>array(
                    ...
                    'option_self_microdata'=>array(
                        'name'=>"Собственная микроразметка опций",
                        'field'=>'option_self_microdata',
                        'help'=>$this->language->get('entry_option_self_microdata_1_help'),
                        'additinal_settings'=>array(
                                'microdata'=>array('element'=>'input','placeholder'=>"Шаблон микроразметки (если находится в другой колонке, укажите её названием для трансформации (в квадратных скобках))",'data-original-title'=>"Укажите свой шаблон через внутренний разделитель, который указывается ниже",'default_value'=>'option_name|option_value_name|price'),
                ...
                'product_special'=>array(
                    'price'=>array(
                        ...
                        'additinal_settings'=>array(
                            ...
                            'price_range'=> array('element'=>'input','range'=>5,'data-original-title'=>$this->language->get('entry_type_data_column_range_price-data-original-title')),
                            'price_around'=>array('element'=>'select'),
                            'customer_group'=>array('element'=>'select','box-title'=>"Группа клиентов брать с сайта"),
                            'customer_group_to_column'=>array('element'=>'input','box-title'=>"Или группа клиентов из другой колонки (укажите её названием для трансформации (в квадратных скобках))"),
                            'priority'=>array('element'=>'input','box-title'=>"Приоритет",'data-original-title'=>"Если находится в другой колонке, укажите её названием для трансформации (в квадратных скобках)",'export'=>0),
                        ),
                    )
                ),
                'product_discount'=>array(
                    'price'=>array(
                        ...
                        'additinal_settings'=>array(
                            ...
                            'price_range'=> array('element'=>'input','range'=>5,'data-original-title'=>$this->language->get('entry_type_data_column_range_price-data-original-title')),
                            'price_around'=>array('element'=>'select'),
                            'customer_group'=>array('element'=>'select','box-title'=>"Группа клиентов"),
                            'customer_group_to_column'=>array('element'=>'input','box-title'=>"Или группа клиентов из другой колонки (укажите её названием для трансформации (в квадратных скобках))"),
                            'priority'=>array('element'=>'input','box-title'=>"Приоритет",'data-original-title'=>"Если находится в другой колонке, укажите её названием для трансформации (в квадратных скобках)",'export'=>0),
                            'quantity'=>array('element'=>'input','box-title'=>"Количество",'export'=>0),
                        )
                    )
                ),
                'attribute'=>array(
    ...
    private function getDsvRowFromOfferNode($offer_row,$only_first_row=FALSE,$offers_file_name_prefix){
                ...
                foreach ($offer_row->param as $yml_param) {
                    ...
                    foreach ($attributes['@attributes'] as $attribute_name => $attribute_value) {
                        ...
                        if($attribute_name=='name' && !in_array((string)$attribute_value, $special_params)){
                            ...
                            if($param_value==='' && isset($attributes['@attributes']['value'])){

                                $param_value = $attributes['@attributes']['value'];
            ...
            if(isset($offer_row->description)){
                ...
                if(isset($offer_row->{'stock-1'}) && isset($offer_row->{'price-1'}) && isset($offer_row->{'size-1'})){

                    for($s=0;$s<15;$s++){

                            if(!$real_price){

                                    $real_price = $this->getValueFromCDATA($offer_row->{'price-'.$s});

                            }

                            if(isset($offer_row->{'stock-'.$s})){

                                    $new_q = 0;

                                    if($offer_row->{'stock-'.$s}=='true'){

                                            $new_q = 10;

                                    }

                                    $params[] = array('title'=>'Вариант','quantity'=>$new_q,'group_title'=>'','value'=>$this->getValueFromCDATA($offer_row->{'size-'.$s}),'price'=>$this->getValueFromCDATA($offer_row->{'price-'.$s}),'image'=>'','feature_code'=>'','product_feature_code'=>'');
                                    
                                    $option_columns[] = 'Вариант';
                                    
                                    unset($offer_row->{'stock-'.$s});
                                    unset($offer_row->{'price-'.$s});
                                    unset($offer_row->{'size-'.$s});
            ...
            elseif(isset($offer_row->{'stock-0'}) && $offer_row->{'price-0'}){

                $real_price = $this->getValueFromCDATA($offer_row->{'price-0'});

                if($offer_row->{'stock-0'}=='true'){

                    $new_q = 10;

                }
                
                unset($offer_row->{'stock-0'});
                unset($offer_row->{'price-0'});
            ...
            if($new_q){
                ...
                $this->yml['offers'][$id]['quantity_by_intro_vars'] = $new_q;
                
            }
            
            if($real_price){
                
                $this->yml['offers'][$id]['price_by_intro_vars'] = $real_price;
    ...
    public function getSupplierSettingView($source_id,$template_data_on_step_one) {
        ...
        if(file_exists(DIR_SYSTEM.'library/vendor/ocext/suppliers/anydsvxls_'.$source_id[0].'.php')){
            ...
            if(isset($template_data_on_step_one['odmpro_tamplate_data']['id'])){
                
                $odmpro_tamplate_data_saved = $this->getSetting('odmpro','odmpro_tamplate_data');
                
                if(isset($odmpro_tamplate_data_saved[$template_data_on_step_one['odmpro_tamplate_data']['id']])){
                    
                    $template_data_on_step_one = $odmpro_tamplate_data_saved[$template_data_on_step_one['odmpro_tamplate_data']['id']];
