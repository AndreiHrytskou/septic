                                                ...
                                                if(isset($additinal_settings['all_product_category']) && $additinal_settings['all_product_category']){
                                                    ...
                                                    if($c == count($categories) && $this->checkColumnTable('product_to_category', 'main_category') && isset($additinal_settings['main_category']) && $additinal_settings['main_category']){
                                                        ...
                                                        if(!empty($product_id)){
                                                            ...
                                                            $sql = "UPDATE " . DB_PREFIX . "product_to_category SET main_category = 0 WHERE product_id = '".$product_id."' ";
                                    ...
                                    if($db_table=='product'){
                                        ...
                                        if($data_to_db_row['product_discount']['rows']){
                                            ...
                                            foreach($data_to_db_row['product_discount']['rows'] as $product_discount_row){
                                                ...
                                                if(empty($additinal_settings['customer_group']) || (isset($additinal_settings['customer_group']) && !empty($additinal_settings['customer_group']) && $additinal_settings['customer_group']==$product_discount_row['customer_group_id'])){
    ...
    public function getFileNameByFileName($file_name,$httpcode=FALSE,$supplier_feed_source='') {
	    ...
	    $xml_catalog_root_tag = FALSE;
            ...
            foreach ($specification as $specification_row) {
                ...
                if(!isset($specification_row['check_valid_status']) || (isset($specification_row['check_valid_status']) && !$specification_row['check_valid_status']) || strstr($yml_check,$specification_row['specification_valid_text']) ){
...
';
	...
	$quick_stock_update_data = array();
        ...
        $import_filter_box = array();
        
        $where_import_filter_box = FALSE;
        
        if(isset($odmpro_tamplate_data['filter_box']['product_data']) && $odmpro_tamplate_data['filter_box']['product_data']){
            
            $import_filter_box = $odmpro_tamplate_data['filter_box']['product_data'];
            
            $where_import_filter_box = $this->getWhere($import_filter_box,' AND ');
                        ...
                        if(!$skip && $filter_import && isset($filter_import[$field]) && $filter_import[$field]){
                            ...
                            foreach ($filter_import[$field] as $filter_import_cond) {
                                ...
                                foreach ($filter_import_cond['value'] as $filter_import_cond_value) {
                                    ...
                                    elseif($filter_import_cond['action']=='skip' && $this->checkValueByOperatorAndFieldValue($csv_value, $filter_import_cond_value, $filter_import_cond['operator']) ){
                ...
                if($skip){
                    ...
                    foreach ($csv_row_whis_data as $csv_position => $csv_value) {
                        ...
                        if(isset($import_data['field_position'][$csv_position])){
                            ...
                            $csv_data_last [$field] = $csv_data[ $field ];
                            ...
                            if(!$skip && isset($csv_data[ $self_column_field ]) && $filter_import && isset($filter_import['self_column___'.$self_column_field]) && $filter_import['self_column___'.$self_column_field]){
                                ...
                                foreach ($filter_import['self_column___'.$self_column_field] as $filter_import_cond) {
                                    ...
                                    foreach ($filter_import_cond['value'] as $filter_import_cond_value) {
                                        ...
                                        elseif($filter_import_cond['action']=='skip' && $this->checkValueByOperatorAndFieldValue($csv_data[ $self_column_field ], $filter_import_cond_value, $filter_import_cond['operator']) ){
			...
			$quick_stock_update_mode = TRUE;
                    ...
                    if($type_change=='update_data' || $type_change=='only_update_data' || $type_change=='only_new_data'){
                        ...
                        foreach ($identificator as $identificator_param) {
                            ...
                            if($identificator_param['identificator_type'] == 'name'){
                                ...
                                $last_data_to_db = $this->getIdByTableAndIdField($identificator_field_name,$identificator_value, $identificator_table, $language_id['value_string'],$where_import_filter_box);
                                ...
                                $last_data_to_db = $this->getIdByTableAndIdField($identificator_field_name,$identificator_value, $identificator_table,0,$where_import_filter_box);
                    ...
                    if(!$skip){
                        ...
                        foreach ($column_settings as $field => $setting) {
                            ...
                            elseif(isset($csv_data[$field])){
                                ...
                                if(!$quick_stock_update_mode && $db_column=='category_whis_path'){
                                    ...
                                    $na_category = md5("__Неизвестная категория__");
                                    ...
                                    if(isset($odmpro_tamplate_data['yandex_market_category_sich'])){
                                        ...
                                        if(isset($csv_data_last) && isset($csv_data_last [$field])){
                                            
                                            $yandex_market_category_sich_category_id = md5($csv_data_last[$field]);
                                        ...
                                        elseif(isset($yandex_market_category_sich[$na_category]) && $yandex_market_category_sich[$na_category]){
                                            
                                            $categories = $yandex_market_category_sich[$na_category];
                                ...
                                elseif(!$quick_stock_update_mode && $db_column=='manufacturer_name'){
                                    ...
                                    if($manufacturer_id && $csv_data[$field]!=='' && isset($general_setting['seo_url_generator']) && $general_setting['seo_url_generator']){

                                        $this->seoUrlGenerateAndSave('manufacturer_id',array($manufacturer_id=>$csv_data[$field]));
                                ...
                                elseif($db_column=='price_advanced'){
                                    ...
                                    if($db_table=='product_discount' || $db_table=='product_special'){
                                        
                                        $product_discount_customer_group_id = $this->config->get('config_customer_group_id');
                                        
                                        
                                        if(!empty($additinal_settings['customer_group_to_column']) && strstr($additinal_settings['customer_group_to_column'], '[[')){
                                            
                                            $product_discount_customer_group_id = (int)$this->getParsFromCollection($additinal_settings['customer_group_to_column'], $csv_data);
                                            
                                        }
                                        elseif(!empty($additinal_settings['customer_group'])){
                                            
                                            $product_discount_customer_group_id = $additinal_settings['customer_group'];
                                            
                                        }
                                        
                                        
                                        $product_discount_quantity = 100;
                                        
                                        if(!empty($additinal_settings['quantity'])){
                                            
                                            $product_discount_quantity = $additinal_settings['quantity'];
                                            
                                        }
                                        
                                        $product_discount_priority = 0;
                                        
                                        if(!empty($additinal_settings['priority']) && strstr($additinal_settings['priority'], '[[')){
                                            
                                            $product_discount_priority = (int)$this->getParsFromCollection($additinal_settings['priority'], $csv_data);
                                            
                                        }
                                        elseif(!empty($additinal_settings['priority'])){
                                            
                                            $product_discount_priority = $additinal_settings['priority'];
                                            
                                        }
                                        
                                        $product_discount_row_key = md5($price.'-'.$product_discount_quantity.'-'.$product_discount_customer_group_id.'-'.$product_discount_priority);
                                        
                                        if($db_table=='product_discount'){
                                            
                                            $new_data_for_db['data'][$db_table][$data_action]['rows'][$product_discount_row_key] = array(
                                                'price' => $price,
                                                'quantity' => $product_discount_quantity,
                                                'customer_group_id' => $product_discount_customer_group_id,
                                                'priority' => $product_discount_priority,
                                            );
                                        ...
                                        if(isset($general_setting['stock_status_id_by_price']) && $general_setting['stock_status_id_by_price'] && empty($price)){
                                        ...
                                        if($attribute_values && $attribute_group_id___attribute_id[1]){
                                            ...
                                            foreach ($attribute_values as $text) {
                                                ...
                                                if($text && isset($attribute_group_id___attribute_id[1])){
                                                    ...
                                                    $id_key = md5($attribute_group_id___attribute_id[1].'-'.$text);
                                                    
                                                    $new_data_for_db['data']['product_attribute'][$data_action]['rows'][$id_key]['text'] = $text;
                                                    
                                                    $new_data_for_db['data']['product_attribute'][$data_action]['rows'][$id_key]['language_id'] = $language_id['value_string'];
                                                    
                                                    $new_data_for_db['data']['product_attribute'][$data_action]['rows'][$id_key]['attribute_id'] = $attribute_group_id___attribute_id[1];
						...
						if($text!==''){
                                                    ...
                                                    $id_key = md5($attribute_id.'-'.$text);
                                                    
                                                    $new_data_for_db['data']['product_attribute'][$data_action]['rows'][$id_key]['text'] = $text;
                                                    
                                                    $new_data_for_db['data']['product_attribute'][$data_action]['rows'][$id_key]['language_id'] = $language_id['value_string'];
                                                    
                                                    $new_data_for_db['data']['product_attribute'][$data_action]['rows'][$id_key]['attribute_id'] = $attribute_id;
                                                    ...
                                                    foreach($attribute_values_whis_attribute_name as $attribute_value_whis_attribute_name){
                                                        ...
                                                        if(isset($attribute_and_value[0]) && isset($attribute_and_value[1])){
                                                            ...
                                                            if($atribute_id && $text!==''){
                                                                ...
                                                                $new_data_for_db['data']['product_attribute'][$data_action]['rows'][$id_key]['attribute_id'] = $atribute_id;
                                        ...
                                        foreach($attribute_values_whis_attribute_name_and_group_name as $attribute_values_whis_attribute_name_and_group_name_row){
                                            ...
                                            if(isset($product_attribute_data[0]) && isset($product_attribute_data[1]) && isset($product_attribute_data[2])){
                                                ...
                                                if($atribute_id && $text!=='' && $atribute_group_id){
                                                    ...
                                                    $id_key = md5($atribute_id.'-'.$text.'-'.$atribute_group_id);
...
$this->setLogDataRow($log_write_row,$log_data);
    ...
    public function getManufacturerIdAndSaveByName($manufacturer_name,$language_id,$store_id,$general_setting,$additinal_settings,$data_action = 'add_values') {
        ...
        $manufacturer_name = explode('_-T-_',$manufacturer_name);
        
        $image = '';
        
        if(isset($manufacturer_name[1]) && $manufacturer_name[1]!==''){
            
            $image = $manufacturer_name[1];
        ...
        if($query->row){
            ...
            $this->db->query(" INSERT INTO `" . DB_PREFIX . "manufacturer` SET image = '".$this->db->escape($image)."', sort_order = 0, name = '".$this->db->escape($manufacturer_name)."' ");
            ...
            elseif($this->showTable('manufacturer_description', DB_PREFIX) && $this->checkColumnTable('manufacturer_description', 'meta_h1')){
             
                $this->db->query(" INSERT INTO `" . DB_PREFIX . "manufacturer_description` SET manufacturer_id = ".$manufacturer_id.", meta_h1 = '".$this->db->escape($manufacturer_name)."', language_id = ".(int)$language_id." ");
                
            }
            elseif($this->showTable('manufacturer_description', DB_PREFIX) && $this->checkColumnTable('manufacturer_description', 'seo_h1')){
             
                $this->db->query(" INSERT INTO `" . DB_PREFIX . "manufacturer_description` SET manufacturer_id = ".$manufacturer_id.", seo_h1 = '".$this->db->escape($manufacturer_name)."', language_id = ".(int)$language_id." ");
    ...
    //'sku',$related_product_sku, 'product', 'product_id'
    ...
    public function updateDataToTable($table,$new_data,$id_name,$id,$aditional_id=array(),$after_delete=FALSE,$delete_data=FALSE){
        ...
        if($this->showTable($table, DB_PREFIX) && $new_data && $id){
			...
			$priority_column_set = FALSE;
			
			foreach($set as $ch_pr_k => $ch_pr){
				
				if(strstr($ch_pr,'priority') && $this->checkColumnTable($table, 'priority')){
					
					$priority_column_set = TRUE;
					
				}
				elseif(strstr($ch_pr,'priority') && !$this->checkColumnTable($table, 'priority')){
            ...
            elseif($table=='product_special' || $table=='product_discount'){
                ...
                if(isset($new_data['price']) && empty($new_data['price'])){

                    return 1;

                }
                
                if(!isset($new_data['customer_group_id']) && $this->checkColumnTable($table, 'customer_group_id')){
                ...
                if(!$priority_column_set && isset($new_data['priority']) && $this->checkColumnTable($table, 'priority')){
                    
                    $set['priority'] = " `priority` = '".(int)$new_data['priority']."' ";
        ...
        elseif($main_table=='order_product'){
            ...
            if($main_data->rows){
                ...
                foreach($main_data->rows as $data){
                    ...
                    foreach ($data as $tname => $tvalue) {
                        $data_to_db[$id][$main_table]['columns'][$tname]['Field'] = $tname;
                    }
 ...
 */
    ...
    public function getLincenceStatus($tmp=TRUE) {
        
        $lic = $this->setting_version->getLincenceStatus($tmp);
    ...
    public function getAnyCSVActionsCache($file_name,$max_age_per_seconds = 30) {
